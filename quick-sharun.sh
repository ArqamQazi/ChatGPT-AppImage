...
_is_deployable_binary() {
	if [ -x "$1" ]; then
		case "$1" in
			*.node) :;;
			*)
				# Statically linked or static-PIE binaries do not need sharun wrappers
				if _is_elf "$1" && ! patchelf --print-interpreter "$1" >/dev/null 2>&1; then
					return 1
				fi
				return 0
				;;
...
_fix_electron_libc_nonsense() {
	...
	# Bundle getconf and ldd in AppDir PATH to satisfy detect-libc without modifying app.asar
	mkdir -p "$DST_BIN_DIR"
	if [ ! -e "$DST_BIN_DIR"/ldd ]; then
		cat <<-'EOF' > "$DST_BIN_DIR"/ldd
#!/bin/sh
echo "ldd (GNU libc) 2.38"
EOF
		chmod +x "$DST_BIN_DIR"/ldd
		_echo "* Added bundled ldd shim (glibc detection)"
	fi

	if [ ! -e "$DST_BIN_DIR"/getconf ]; then
		cat <<-'EOF' > "$DST_BIN_DIR"/getconf
#!/bin/sh
case "$1" in
...
EOF
		chmod +x "$DST_BIN_DIR"/getconf
		_echo "* Added bundled getconf shim (glibc detection)"
	fi

	# Ensure LIBC=glibc is set in the runtime environment for node-gyp-build
	echo 'LIBC=glibc' >> "$APPENV"
}
